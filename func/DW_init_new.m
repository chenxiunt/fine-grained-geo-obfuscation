function [GeoIMatrix, extremepoint] = DW_init_new(peerMatrix, peerNeighbor, distMatrix, costMatrix, k, NR_LOC, EPSILON, iter)
    extremepoint = []; 
...
%     lb = zeros(NR_LOC, 1);
%     ub = zeros(NR_LOC, 1);
%     ub(real_loc_idx, 1) = ones(size(real_loc_idx)); 
%     f = costMatrix(:, k);
